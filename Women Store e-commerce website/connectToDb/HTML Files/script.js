...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function login(email, password) {
        ...
        .then(response => {
            if (response.ok) {
                // If the response is successful (status code 200), perform your desired actions
                console.log('Login successful');
                // You can redirect to another page or update the UI as needed
            } else {
                // If the response is not successful, handle the error
                console.error('Login failed');
                alert('Invalid email or password'); // Show an alert or update the UI to indicate login failure
            }
        })
        .catch(error => {
            // Handle network errors or other fetch-related issues
            console.error('Error during login:', error);
            alert('An error occurred during login. Please try again.'); // Show an alert or update the UI to indicate an error
        });
    }
});
